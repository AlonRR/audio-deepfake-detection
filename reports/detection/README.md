...
`*_scores.npz` (raw scores+labels for DET curves). Figures live in `../figures/`
(prefixed by run). Regenerate with `scripts/plot_detection.py`; refresh from the lab with
`scripts/fetch_results.sh`. Full timeline + wall-times + failures: `docs/runtimes.md`.
